    ...
    <div className="h-full flex flex-col">
      ...
      <Card className="bg-gradient-to-br from-purple-50 to-pink-50 border-2 border-purple-200 shadow-lg h-full flex flex-col">
        ...
        <CardContent className="pt-0 flex-1 flex flex-col space-y-2">
          ...
          <div className="flex-shrink-0">
            ...
            <div className="grid grid-cols-2 gap-1">
              <label className="flex items-center space-x-1 cursor-pointer">
                <input type="radio" name="mode" value="start" checked={mode === 'start'} onChange={(e) => onModeChange(e.target.value as PlacementMode)} className="w-3 h-3 text-purple-600" />
                <div className="w-2 h-2 cell-start rounded"></div>
                <span className="text-xs text-purple-700">Start</span>
              </label>
              <label className="flex items-center space-x-1 cursor-pointer">
                <input type="radio" name="mode" value="end" checked={mode === 'end'} onChange={(e) => onModeChange(e.target.value as PlacementMode)} className="w-3 h-3 text-purple-600" />
                <div className="w-2 h-2 cell-end rounded"></div>
                <span className="text-xs text-purple-700">End</span>
              </label>
              <label className="flex items-center space-x-1 cursor-pointer">
                <input type="radio" name="mode" value="obstacle" checked={mode === 'obstacle'} onChange={(e) => onModeChange(e.target.value as PlacementMode)} className="w-3 h-3 text-purple-600" />
                <div className="w-2 h-2 cell-obstacle rounded"></div>
                <span className="text-xs text-purple-700">Wall</span>
              </label>
              <label className="flex items-center space-x-1 cursor-pointer">
                <input type="radio" name="mode" value="erase" checked={mode === 'erase'} onChange={(e) => onModeChange(e.target.value as PlacementMode)} className="w-3 h-3 text-purple-600" />
          ...
          <div className="flex-shrink-0">
            <Label className="text-xs font-medium text-purple-700 mb-1 block">⚡ Speed: {speedNames[speed as keyof typeof speedNames]}</Label>
            <Slider
              value={[speed]}
          ...
          <div className="flex-1 min-h-0">
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs font-mono bg-white/50 px-1 py-0.5 rounded text-purple-700 truncate max-w-20">{stats.status}</span>
              </div>
              <div className="grid grid-cols-3 gap-1 text-xs">
                <div className="text-center">
                  <div className="text-purple-600">Explored</div>
                  <div className="font-mono bg-purple-100 px-1 rounded text-purple-800">{stats.exploredCount}</div>
                </div>
                <div className="text-center">
                  <div className="text-purple-600">Path</div>
                  <div className="font-mono bg-pink-100 px-1 rounded text-purple-800">{stats.pathLength}</div>
                </div>
                <div className="text-center">
                  <div className="text-purple-600">Time</div>
                  <div className="font-mono bg-purple-100 px-1 rounded text-purple-800">{stats.timeElapsed}ms</div>
                </div>
              </div>
            </div>
          </div>
